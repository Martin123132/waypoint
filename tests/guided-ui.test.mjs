...
async function run() {
  ...
  try {
    ...
    await detailPanel.getByText('Destination valid').waitFor({ timeout: 10000 })
    await detailPanel.getByText('Fallback path', { exact: true }).waitFor({ timeout: 10000 })
    await detailPanel.getByLabel('Route checks').getByText('Updated', { exact: false }).waitFor({ timeout: 10000 })
    ...
    await detailPanel.getByLabel('Route health').getByText('Unsaved edits').waitFor({ timeout: 10000 })
    await detailPanel.getByText('Save this draft to update the redirect and QR record.').waitFor({ timeout: 10000 })
    await detailPanel.locator('.button-row').getByRole('button', { name: 'Save changes' }).click()
    await detailPanel.getByLabel('Link status').getByText('Saved', { exact: true }).waitFor({ timeout: 10000 })
    await detailPanel.getByText('Ready to share').waitFor({ timeout: 10000 })
    ...
    await detailPanel.getByText('Brand path available').waitFor({ timeout: 10000 })
    await detailPanel.getByText(`Apply ${host} to make the public URL cleaner.`).waitFor({ timeout: 10000 })
    ...
    await detailPanel.getByText('Branded path').waitFor({ timeout: 10000 })
    await detailPanel.getByText('Ready to share').waitFor({ timeout: 10000 })
